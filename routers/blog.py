...
@router.get("/")
def get_all_blogs(db:Session = Depends(get_db),current_user = Depends(get_current_user)):
...
def update_blog(id:int,request:Blog,db:Session = Depends(get_db),current_user = Depends(get_current_user)):
...
def get_blog_by_id(id:int,db:Session = Depends(get_db),current_user = Depends(get_current_user)):
